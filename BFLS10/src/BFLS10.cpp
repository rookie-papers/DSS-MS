...
static void BFLS10_Setup(benchmark::State &state) {
    initRNG(&rng_BFLS10);
    initState(state_BFLS10);
    mpz_class gamma = rand_mpz(state_BFLS10);
    for (auto _: state) {
...
static void BFLS10_KeyGen(benchmark::State &state) {
    initRNG(&rng_BFLS10);
    initState(state_BFLS10);
    mpz_class gamma = rand_mpz(state_BFLS10);
    // Setup
...
static void BFLS10_Sign(benchmark::State &state) {
    initRNG(&rng_BFLS10);
    initState(state_BFLS10);
    mpz_class gamma = rand_mpz(state_BFLS10);
    // Setup
    ...
    ECP_copy(&PK_san, &KP_BBS_san.A);
    for (auto _: state) {
        Signing(KP_schnorr_sig, m_FIX, pp_S, KP_BBS_sig, m, PK_san, ADM, gamma);
    }
}

static void BFLS10_Sanitizing(benchmark::State &state) {
    initRNG(&rng_BFLS10);
    initState(state_BFLS10);
    mpz_class gamma = rand_mpz(state_BFLS10);
    // Setup
...
static void BFLS10_Verify(benchmark::State &state) {
    initRNG(&rng_BFLS10);
    initState(state_BFLS10);
    mpz_class gamma = rand_mpz(state_BFLS10);
    // Setup
    ...
    mpz_class msgDigest_FULL = ComDigest_FULL(m, PK_san);
    for (auto _: state) {
        Verification(pp_S, KP_schnorr_sig.PK, msgDigest_FIX, msgDigest_FULL, sigma);
    }
}

void BFLS10_Proof(benchmark::State &state) {
    initRNG(&rng_BFLS10);
    initState(state_BFLS10);
    mpz_class gamma = rand_mpz(state_BFLS10);
    // Setup
    ...
    res = Verification(pp_S, KP_schnorr_sig.PK, msgDigest_FIX, msgDigest_FULL_p, sigma_p);
    for (auto _: state) {
        Proof(pp_S, KP_schnorr_sig.PK, sigma, msgDigest_FIX, sigma.gpk);
    }
}

void BFLS10_Judge(benchmark::State &state) {
    initRNG(&rng_BFLS10);
    initState(state_BFLS10);
    mpz_class gamma = rand_mpz(state_BFLS10);
    // Setup
...
// register
BENCHMARK(BFLS10_Setup);
BENCHMARK(BFLS10_KeyGen);
BENCHMARK(BFLS10_Sign);
BENCHMARK(BFLS10_Sanitizing);
BENCHMARK(BFLS10_Verify);
BENCHMARK(BFLS10_Proof);
BENCHMARK(BFLS10_Judge);
